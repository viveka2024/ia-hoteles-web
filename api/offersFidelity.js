...
export default async function handler(req, res) {
  ...
  if (!hotel_id) return res.status(400).json({ error: 'Falta hotel_id' });
  ...
  if (req.method === 'PATCH') {
    const { id } = req.body;
    if (!id) return res.status(400).json({ error: 'Falta id de la oferta' });
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from('fidelity_offers')
        .insert([{ hotel_id, text, embedding: vector, expires_at: expires_at || null }])
        .select();

      if (error) throw error;
      return res.status(201).json({ offer: data[0] });
    } catch (err) {
      console.error('offersFidelity.js error:', err);
